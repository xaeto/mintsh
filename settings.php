...
if ($hassiteconfig || has_capability('theme/boost_union:configure', context_system::instance())) {
    ...
    $mainsettingspageurl = new moodle_url('/admin/settings.php', ['section' => 'themesetting_mintsh']);
    if ($ADMIN->fulltree && $PAGE->has_set_url() && $PAGE->url->compare($mainsettingspageurl)) {
        redirect(new moodle_url('/admin/settings.php', ['section' => 'theme_mintsh']));
    }
    ...
    if (!$ADMIN->fulltree) {
        ...
        $tab = new admin_settingpage('theme_mintsh',
            get_string('configtitle', 'theme_mintsh', null, true),
    ...
    // @codingStandardsIgnoreLine
        ...
        require_once($CFG->dirroot . '/theme/mintsh/lib.php');
        require_once($CFG->dirroot . '/theme/mintsh/locallib.php');
        ...
        // (and allow users with the theme/boost_union:configure capability to access it).
        $page = new theme_boost_admin_settingspage_tabs('theme_mintsh',
            get_string('configtitle', 'theme_mintsh', null, true),
            'theme/boost_union:configure');


        // Create general settings tab.
        $tab = new admin_settingpage('theme_mintsh_general',
            get_string('generalsettings', 'theme_boost', null, true));

        // Create inheritance heading.
        $name = 'theme_mintsh/inheritanceheading';
        $title = get_string('inheritanceheading', 'theme_mintsh', null, true);
        $setting = new admin_setting_heading($name, $title, null);
        $tab->add($setting);

        // Prepare inheritance options.
        $inheritanceoptions = [
            THEME_MINTSH_SETTING_INHERITANCE_INHERIT =>
                get_string('inheritanceinherit', 'theme_mintsh'),
            THEME_MINTSH_SETTING_INHERITANCE_DUPLICATE =>
                get_string('inheritanceduplicate', 'theme_mintsh'),
        ];

        // Setting: Pre SCSS inheritance setting.
        $name = 'theme_mintsh/prescssinheritance';
        $title = get_string('prescssinheritancesetting', 'theme_mintsh', null, true);
        $description = get_string('prescssinheritancesetting_desc', 'theme_mintsh', null, true).'<br />'.
            get_string('inheritanceoptionsexplanation', 'theme_mintsh', null, true);
        $setting = new admin_setting_configselect($name, $title, $description,
            THEME_MINTSH_SETTING_INHERITANCE_INHERIT, $inheritanceoptions);
        $setting->set_updatedcallback('theme_reset_all_caches');
        $tab->add($setting);

        // Setting: Extra SCSS inheritance setting.
        $name = 'theme_mintsh/extrascssinheritance';
        $title = get_string('extrascssinheritancesetting', 'theme_mintsh', null, true);
        $description = get_string('extrascssinheritancesetting_desc', 'theme_mintsh', null, true).'<br />'.
            get_string('inheritanceoptionsexplanation', 'theme_mintsh', null, true);
        $setting = new admin_setting_configselect($name, $title, $description,
            THEME_MINTSH_SETTING_INHERITANCE_INHERIT, $inheritanceoptions);
        $setting->set_updatedcallback('theme_reset_all_caches');
